QSPhone.RegisterServerCallback('qs-smartphone:server:GetInvoices', function(source, cb)
            ...
            if invoices[1] ~= nil then
                for k, v in pairs(invoices) do
                    ...
                    if Ply ~= nil then
                        ...
                        local resultNumber = MySQL.Sync.fetchAll("SELECT charinfo FROM `users` WHERE `identifier` = '"..v.sender.."'", {} )
                ...
                for k, v in pairs(invoices) do
                    if v.status == 'unpaid' then
                        ...
                        if Ply ~= nil then
                            ...
                            local resultNumber = MySQL.Sync.fetchAll('SELECT charinfo FROM users WHERE identifier = @id', { ['@id'] = v.author_identifier } )
                            if resultNumber and resultNumber[1] then 
                                v.number = json.decode(resultNumber[1].charinfo).phone
                            end
                        end
    ...
    elseif Config.Framework == 'QBCore' then 
        ...
        if Config.billingSystem == 'default' or Config.billingSystem == 'jim-payments' then
            ...
            if invoices[1] ~= nil then
                for _, v in pairs(invoices) do
                    ...
                    if Ply ~= nil then
                        ...
                        local res = MySQL.Sync.fetchAll('SELECT charinfo FROM players WHERE citizenid = ?', {v.sendercitizenid})
                        if res[1] ~= nil then
                ...
                for k, v in pairs(invoices) do
                    if v.status == 'unpaid' then
                        ...
                        if Ply ~= nil then
                            ...
                            local resultNumber = MySQL.Sync.fetchAll('SELECT charinfo FROM players WHERE citizenid = @citizenid', { ['@citizenid'] = v.author_identifier } )
                            if resultNumber and resultNumber[1] then 
                                v.number = json.decode(resultNumber[1].charinfo).phone
                            end
                        end
    ...
    if Config.billingSystem == 'jim-payments' then
        ...
        if invoice and invoice[1] then
            ...
            if SenderPly then
                ...
                -- Disconnected Player
                MySQL.Async.fetchAll("SELECT * FROM `players` WHERE citizenid = @citizenid", { ['@citizenid'] = invoice[1].sendercitizenid }, function(result)
                    if result and result[1] then
                        Ply.Functions.RemoveMoney('bank', invoice[1].amount)
                        local decoded = json.decode(result[1].money)
                        decoded.bank = decoded.bank + invoice[1].amount
                        MySQL.Async.execute("UPDATE `players` SET `money` = '"..json.encode(decoded).."' WHERE `citizenid` = '"..invoice[1].sendercitizenid.."'", {}) 
                        MySQL.Async.execute('DELETE FROM phone_invoices WHERE id = ?', {invoiceId})
                        cb(true)
                    end
                end)
